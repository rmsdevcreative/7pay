...
class CashOutPinVerificationPage extends StatelessWidget {
  const CashOutPinVerificationPage({super.key, required this.context});
  ...
  Widget _buildContactTile(
    ...
    return CustomContactListTileCard(
      leading: MyNetworkImageWidget(
        ...
        isProfile: true,
        imageUrl: '${controller.existAgentModel?.getAgentImageUrl()}',
        imageAlt: '${controller.existAgentModel?.getFullName()}',
      ),
      padding: padding ?? EdgeInsetsDirectional.zero,
      imagePath: controller.existAgentModel?.getAgentImageUrl(),
      title: controller.existAgentModel?.getFullName(),
      subtitle: "+${controller.existAgentModel?.getUserMobileNo(withCountryCode: true)}",
      showBorder: showBorder,
    );
  }

  // Reusable Amount Details Card
  Widget _buildAmountDetailsCard(CashOutController controller) {
  ...
  Future<void> _showConfirmDialog(CashOutController controller) async {
