...
class DonationPinVerificationPage extends StatelessWidget {
  const DonationPinVerificationPage({super.key, required this.context});
  ...
  Widget _buildAmountDetailsCard(DonationController controller) {
    return AmountDetailsCard(
      amount: MyUtils.getUserAmount(
        controller.amountController.text.toString(),
      ),
      charge: '+0.0',
      total: MyUtils.getUserAmount(controller.amountController.text.toString()),
    );
  }

  // Reusable Confirm Dialog
  Future<void> _showConfirmDialog(DonationController controller) async {
